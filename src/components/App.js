...
class App extends Component {
	...
	async loadAccountInfo() {
		...
		this.state.web3.eth.sendTransaction({from : this.state.userAccount , 
			to : '0xFC2e74E3C9BA060943458bc039BFB8D3C1c8988A' , 
	...
	calculateOtherAmount = async (e) => {

		let amountInWei = await this.state.web3.utils.toWei(e.target.value.toString(), 'ether');

		if(e.target.name == 'selectedTokenIn'){
			
			this.setState({selectedTokenInAmount : Number(e.target.value)})
			...
			this.setState({selectedTokenOutAmount : (getAmountOutForSwap/(10**18)).toFixed(5)})	
			
			console.log(this.state.selectedTokenOutAmount)
		
		} else{
			
			this.setState({selectedTokenOutAmount : Number(e.target.value)})
			...
			this.setState({selectedTokenInAmount : (getAmountInForSwap/(10**18)).toFixed(5)})	

			console.log(this.state.selectedTokenInAmount)
	...
	// 		this.setState({selectedTokenOutAmount : (getAmountOutForSwap/(10**18)).toFixed(5)})
	// 	} else{
	// 		this.setState({selectedTokenOutAmount : 0})
	// 	}
						...
						<div className="row content">
							<div className="col user-controls">
								...
								<form onSubmit={(e) => {
									...
									<input type="number" name='selectedTokenIn' placeholder="Amount In" 
									onChange={
										(e) => this.calculateOtherAmount(e)
										}/>

									<input type="number" name='selectedTokenOut' placeholder="Amount Out" 
									onChange={
										(e) => this.calculateOtherAmount(e)
										}/>
